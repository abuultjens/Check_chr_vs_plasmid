...
#	    MOB_RECON=`tail -1 ${RAND}_${TAXA}_${CONTIG}_mob_recon/contig_report.txt | cut -f 2`
        ...
        echo "${TAXA},${CONTIG},${CHR_READS},${P_READS},${CLASSIFICATION}" >> ${RAND}_report.csv
    } &
...
echo "CHROMOSOME_READS_MAPPED,PLASMID_READS_MAPPED,CLASSIFICATION,DIFFERENCE" > ${RAND}_ordered_report.csv
for TAXA in $(cat ${RAND}_ID_fofn.txt); do
	...
	if [ "$CHR" == "NA" ]; then
		...
		DIFF=$(awk -v chr="$CHR" -v p="$P" 'BEGIN {print (p > chr ? p - chr : chr - p)}')
	fi

	echo "${CHR},${P},${CLASSIFICATION},${DIFF}" >> ${RAND}_ordered_report.csv
done
